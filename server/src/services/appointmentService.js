...
const appointmentService = {
    ...
    updateAppointmentStatus: async (id, status, paymentMethod) => {
        ...
        const cancelled = await appointmentRepository.cancel(id)
        if (!cancelled) {
    ...
    updateAppointment: async (id, appointmentData) => {
        ...
        const current = await appointmentRepository.findByIdWithDetails(id)
        if (!current) {
            throw new Error('Agendamento não encontrado.')
        }

        // Se a data ou hora mudou, verificar disponibilidade
        if (appointmentData.date !== current.date || appointmentData.time !== current.time) {
            const isBusy = await appointmentRepository.checkAvailability(
                current.barber_id,
                appointmentData.date,
                appointmentData.time
        ...
        const updated = await appointmentRepository.update(id, appointmentData)
        if (!updated) {
            throw new Error('Agendamento não pode ser atualizado.')
        }
